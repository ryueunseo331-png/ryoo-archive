...
// 처음 만난 날짜 (D+ 계산용)
const START_DATE = new Date(2026, 4, 3); // 로컬 자정 기준 (문자열로 파싱하면 UTC 자정이 되어 시간대만큼 밀림)
...
async function uploadPhoto(file) {
  try {
    ...
    try {
      ...
      blob = file; // 변환에 실패하면 원본이라도 그대로 업로드
    }
    // 파일 자체에 형식 정보를 확실히 박아넣음 (업로드 옵션만으로는 서버에 안 먹히는 경우가 있어서)
    const jpegBlob = new Blob([blob], { type: 'image/jpeg' });
    const filename = `${currentRole}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.jpg`;
    const { error } = await sb.storage.from('photos').upload(filename, jpegBlob, {
      cacheControl: '3600',
      upsert: false,
      contentType: 'image/jpeg',
    });
    if (error) {
      alert('사진 하나를 업로드하지 못했어요: ' + error.message);
      return null;
    }
    const { data } = sb.storage.from('photos').getPublicUrl(filename);
    return data.publicUrl;
  } catch (e) {
    // 여기서 무슨 일이 생기든 절대 던지지 않음 — 사진 하나가 실패해도
    // 나머지 저장(일기 텍스트 등)까지 같이 날아가지 않도록
    alert('사진 하나를 업로드하지 못했어요: ' + e.message);
    return null;
  }
}

// 어떤 형식(HEIC 포함)으로 찍힌 사진이든 모든 브라우저가 볼 수 있는 JPEG로 변환하고,
// 너무 큰 사진은 적당히 줄여서 용량도 아낌
function compressImage(file) {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => { clearTimeout(timer); URL.revokeObjectURL(url); reject(new Error('이미지를 열 수 없어요')); };
    img.src = url;
  });
}
...
async function loadAllData() {
  const [myRecs, theirRecs, posts, letters, thoughts, tlPast, tlFuture, hero, profile, notifs] = await Promise.all([
    ...
    sb.from('hero_photo').select('*').eq('user_type', currentRole).order('updated_at', { ascending: false }).limit(1),
...
function parseTlDate(str) {
  ...
  if (!m) return null;
  return { year: parseInt(m[1], 10), month: parseInt(m[2], 10), day: m[3] ? parseInt(m[3], 10) : null };
}
...
async function migratePastDueFuturePlans() {
  ...
  cache.timelineFuture.forEach(item => {
    ...
    if (parsed) {
      // 일자가 없으면(예: "2026. 08") 그 달이 통째로 지나야 과거로 넘김
      const itemYMD = parsed.day
        ? parsed.year * 10000 + parsed.month * 100 + parsed.day
        : parsed.year * 10000 + parsed.month * 100 + 32;
      if (itemYMD < todayYMD) { toMigrate.push(item); return; }
    }
...
async function confirmHeroFocus() {
  ...
  const { error } = await sb.from('hero_photo').insert({
    photo_url: url, focal_x: heroFocusXY.x, focal_y: heroFocusXY.y, user_type: currentRole,
  });
  if (error) {
...
function selectDay(el, dateStr, who) {
  ...
  if (who === 'their' && record) incrementReadCount('records', record);
}

async function incrementReadCount(table, row) {
  const newCount = (row.read_count || 0) + 1;
  const { error } = await sb.from(table).update({ read_count: newCount }).eq('id', row.id);
  if (!error) row.read_count = newCount;
}
  ...
  detail.innerHTML = `
    <div class="day-detail-header">
      <div style="display:flex;align-items:baseline;gap:8px;">
        ...
        ${canEdit && record?.read_count ? `<span class="day-detail-time"><i class="ti ti-eye" style="font-size:10px"></i> ${record.read_count}</span>` : ''}
      </div>
      ${canEdit ? `<i class="ti ti-pencil" style="font-size:14px;color:var(--text-tertiary);cursor:pointer" onclick="openEditDiary('${dateStr}')"></i>` : ''}
    </div>
    ${(photos.length || canEdit) ? `<div class="day-photos-wrap"><div class="day-photos">${photosHtml}</div>${(photos.length + (canEdit ? 1 : 0)) > 1 ? '<div class="day-photos-more"><i class="ti ti-chevron-right"></i></div>' : ''}</div>` : ''}
...
function openAddRecord() {
  const today = new Date();
  document.getElementById('recordDate').value = toLocalDateStr(today);
  ...
  document.getElementById('recordTimeGroup').classList.add('hidden');
  document.getElementById('recordExistingPhotosGroup').classList.add('hidden');
  document.getElementById('recordDeleteBtn').classList.add('hidden');
  editingPhotosDraft = [];
  openModal('modal-record');
}

let editingPhotosDraft = [];
...
function openEditDiary(dateStr) {
  ...
  document.getElementById('recordTimeGroup').classList.remove('hidden');
  document.getElementById('recordDeleteBtn').classList.remove('hidden');
  ...
  editingPhotosDraft = [...(record.photos || [])];
  renderRecordExistingPhotos();

  openModal('modal-record');
}

function renderRecordExistingPhotos() {
  const group = document.getElementById('recordExistingPhotosGroup');
  const wrap = document.getElementById('recordExistingPhotos');
  if (editingPhotosDraft.length === 0) {
    group.classList.add('hidden');
    wrap.innerHTML = '';
    return;
  }
  group.classList.remove('hidden');
  wrap.innerHTML = editingPhotosDraft.map((p, i) => `
    <div class="record-existing-photo">
      <img src="${p}" alt="사진">
      <span class="record-existing-photo-remove" onclick="removeExistingPhoto(${i})">×</span>
    </div>`).join('');
}

function removeExistingPhoto(idx) {
  editingPhotosDraft.splice(idx, 1);
  renderRecordExistingPhotos();
}

async function deleteRecord() {
  const editDate = document.getElementById('modal-record').dataset.editDate;
  if (!editDate) return;
  if (!confirm('이 날짜의 기록을 삭제할까요? 사진과 댓글도 함께 사라져요.')) return;

  const list = getRecordsFor(currentRole);
  const existing = list.find(r => r.date === editDate);
  if (!existing) return;

  const { error } = await sb.from('records').delete().eq('id', existing.id);
  ...
  renderCalendar();
  renderRecentEntries();
  renderDayDetail(editDate, null, 'dayDetail', true);
}
...
async function saveRecord() {
  ...
  try {
    ...
    if (editDate) {
      // 기존 기록 수정 (날짜/시간 변경, 사진 삭제, 일기 수정)
      const existing = list.find(r => r.date === editDate);
      if (!existing) throw new Error('기록을 찾을 수 없어요');

      const timeVal = document.getElementById('recordTime').value || '00:00';
      const [hh, mm] = timeVal.split(':').map(Number);
      const newCreatedAt = new Date(date + 'T00:00:00');
      newCreatedAt.setHours(hh, mm, 0, 0);

      const { error } = await sb.from('records').update({
        date, diary, photos: editingPhotosDraft, created_at: newCreatedAt.toISOString(),
      }).eq('id', existing.id);
      if (error) throw error;

      existing.date = date;
      existing.diary = diary;
      existing.photos = editingPhotosDraft;
      existing.created_at = newCreatedAt.toISOString();
      ...
      renderDayDetail(date, existing, 'dayDetail', true);
    } else {
      // 새 기록 추가
      let photoUrls = [];
      if (files.length > 0) {
      ...
      renderDayDetail(date, data, 'dayDetail', true);
    }
  } catch (e) {
...
function openPostView(id) {
  ...
  const readSuffix = (post.user_type === currentRole && post.read_count) ? ' · 읽음 ' + post.read_count : '';
  document.getElementById('postViewMeta').textContent = authorNameOf(post.user_type) + ' · ' + formatDateShort(new Date(post.created_at)) + readSuffix;
  ...
  if (post.user_type !== currentRole) incrementReadCount('board_posts', post);
}
...
function renderThoughts() {
  ...
  const thoughts = cache.thoughts
    ...
    .sort(() => Math.random() - 0.5); // 순서를 매번 랜덤하게 섞음
  ...
  // 캔버스가 아직 화면에 안 보이는 상태(크기 0)라면 안전하게 배치할 수 없으니
  // 화면에 그려질 때까지 기다렸다가 다시 시도함
  if (canvasW < 20 || canvasH < 20) {
    requestAnimationFrame(() => requestAnimationFrame(renderThoughts));
    return;
  }
  ...
  thoughts.forEach(t => {
    ...
    const maxLeft = Math.max(pad, canvasW - w - pad);
    const maxTop = Math.max(pad, canvasH - h - pad);

    let left = pad, top = pad;
    for (let i = 0; i < 100; i++) {
      const tryLeft = pad + Math.random() * (maxLeft - pad);
      const tryTop = pad + Math.random() * (maxTop - pad);
      const overlaps = placed.some(p =>
        tryLeft < p.left + p.width + 8 && tryLeft + w + 8 > p.left &&
        tryTop < p.top + p.height + 8 && tryTop + h + 8 > p.top
      );
      if (!overlaps) { left = tryLeft; top = tryTop; break; }
      left = tryLeft; top = tryTop; // 못 찾아도 마지막 시도 위치라도 사용
    }
...
async function openLetter(letter) {
  ...
  showPage('letter-view', 'menu-letter');
  renderLetters();
}
...
async function downloadCurrentLbPhoto() {
  const item = lbItems[lbIdx];
  ...
  try {
    ...
    a.href = blobUrl;
    const dateLabel = item.date ? item.date.replace(/-/g, '') : Date.now();
    a.download = 'ryoo-archive-' + dateLabel + '.jpg';
    document.body.appendChild(a);
    a.click();
    a.remove();
    setTimeout(() => URL.revokeObjectURL(blobUrl), 5000);
  } catch (e) {
    // 자동 다운로드가 막히면, 새 탭에서 열어서 직접 저장할 수 있게 함
...
function attachSwipeNav(el, onSwipeLeft, onSwipeRight) {
  ...
  el.addEventListener('touchend', function(e) {
    ...
    if (Math.abs(dx) < 50 || Math.abs(dx) < Math.abs(dy) * 1.3) return; // 세로 스크롤과 헷갈리지 않게
    if (dx < 0) onSwipeLeft(); else onSwipeRight();
  }, { passive: true });
}

attachSwipeNav(document.getElementById('thoughtsCanvas'), () => changeThoughtsMonth(1), () => changeThoughtsMonth(-1));
attachSwipeNav(document.getElementById('calGrid'), () => changeMonth(1), () => changeMonth(-1));
attachSwipeNav(document.getElementById('calGridTheir'), () => changeMonthTheir(1), () => changeMonthTheir(-1));
